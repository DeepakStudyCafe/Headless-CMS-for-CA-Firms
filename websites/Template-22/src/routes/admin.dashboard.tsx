...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (!token) { navigate({ to: "/admin/login", replace: true }); return; }
    Promise.all([
    ...
    ]).catch(() => { localStorage.removeItem("site_admin_token"); navigate({ to: "/admin/login", replace: true }); })
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      setTimeout(() => { localStorage.removeItem("site_admin_token"); navigate({ to: "/admin/login", replace: true }); }, 1500);
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        ...
        {activeTab === "pages" && (
          <div className="space-y-3">
            ...
            {pages.map((page) => (
              <button key={page.id} type="button" onClick={() => navigate({ to: `/admin/pages/${page.id}` })}
        ...
        {activeTab === "footer" && (
          <div className="rounded-xl border p-6 space-y-5" style={{ background: "#111111", borderColor: "rgba(224,140,46,0.15)" }}>
            ...
            <div className="space-y-4">
              ...
              <div><label className={lbl} style={lblStyle}>Copyright Text</label><input type="text" value={footerForm.copyright} onChange={(e) => setFooterForm((f) => ({ ...f, copyright: e.target.value }))} placeholder="© 2026 ABC & Associates." className={inp} style={inpStyle} /></div>
            </div>
