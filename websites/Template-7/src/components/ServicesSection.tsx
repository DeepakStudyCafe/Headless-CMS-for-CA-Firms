  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mt-6">
        <a
          href={service.href || '#'}
          className="font-body text-[12px] font-medium transition-all duration-300 group-hover:translate-x-1"
          style={{ color: '#C8A96E' }}
        >
          Learn More
        </a>
...
export default function ServicesSection({ data }: { data?: any }) {
  ...
  const label = tc.label || 'PRACTICE AREAS';
  const heading = tc.heading || 'What We Do Best';
  const services = (tc.items || DEFAULT_SERVICES).map((s: any) => ({
    title: s.title || '',
