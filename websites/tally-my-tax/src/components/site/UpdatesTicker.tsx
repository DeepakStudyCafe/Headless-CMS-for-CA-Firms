...
export function UpdatesTicker({ posts }: Props) {
  ...
  const tickerBg = "bg-card border border-primary/20 shadow-[0_4px_20px_rgba(var(--primary),0.15)] rounded-xl overflow-hidden"
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex items-center justify-center gap-2 py-4 px-3 bg-primary/5 border-b border-primary/20 flex-shrink-0">
        <span className="w-2 h-2 rounded-full bg-primary animate-pulse" aria-hidden="true" />
        <h3 className="text-xs font-bold tracking-widest text-primary uppercase select-none">{`What's New`}</h3>
        <span className="w-2 h-2 rounded-full bg-primary animate-pulse" aria-hidden="true" />
      </div>
      ...
      <div
        ...
        className="flex-1 overflow-y-scroll min-h-[300px] bg-background"
      ...
      >
        {searching && (
          <div className="flex items-center justify-center h-20 gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-primary/70 animate-bounce" style={{ animationDelay: '0ms' }} />
            <span className="w-2.5 h-2.5 rounded-full bg-primary/70 animate-bounce" style={{ animationDelay: '150ms' }} />
            <span className="w-2.5 h-2.5 rounded-full bg-primary/70 animate-bounce" style={{ animationDelay: '300ms' }} />
          </div>
        )}
        ...
        {!searching && (isSearchMode ? displayPosts : [...displayPosts, ...displayPosts]).map((post, idx) => (
          <div key={`${post.id}-${idx}`} role="listitem"
            ...
            <p className="text-[10px] font-bold text-primary/90 tracking-wide mb-1 select-none">
              {formatDate(post.date)}
            </p>
            <Link
              to={`/post/${post.slug}`}
              className="text-sm text-foreground/80 hover:text-primary font-medium leading-snug transition-colors focus:outline-none focus:text-primary line-clamp-3 block decoration-transparent"
      ...
      {/* Search bar */}
      <div className="border-t border-primary/20 px-3 py-3 flex-shrink-0 bg-secondary/20">
        <div className="relative">
          <input
            ...
            className="w-full bg-background text-foreground text-xs placeholder:text-muted-foreground rounded-md pl-8 pr-8 py-2.5 border border-border/50 focus:border-primary focus:ring-1 focus:ring-primary/30 focus:outline-none transition-all"    
          />
        ...
        {!query && (
          <button
            ...
            className={`mt-2 w-full text-[10px] font-semibold tracking-wider uppercase py-1.5 rounded-md transition-colors ${paused ? 'bg-primary/20 text-primary' : 'text-muted-foreground hover:text-foreground hover:bg-secondary'}`}
          >
            {paused ? '▶ Resume' : '⏸ Pause'}
          </button>
        )}
