...
const Contact = () => {
  ...
  return (
    <Layout>
      <PageHero
        title={heroSection?.textContent?.heading?.split(' ').slice(0, -1).join(' ') || "Let's"}
        highlight={heroSection?.textContent?.heading?.split(' ').pop() || "Connect"}
        subtitle={heroSection?.textContent?.subheading || cc.heroSubtitle || "Ready to discuss how we can help your business? We'd love to hear from you."}
        image={heroContact}
        ...
        <div className="container-max mx-auto">
          <ScrollReveal>
            ...
            <Button variant="gold" size="lg" asChild>
              <a href={`tel:${(websiteData?.phone || '+911234567890').replace(/\s/g, '')}`}><Phone className="w-4 h-4 mr-2" /> Call Us Now</a>
            </Button>
          </ScrollReveal>
        </div>
      </section>
