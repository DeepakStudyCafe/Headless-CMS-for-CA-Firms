...
export default function AdminDashboard() {
  ...
  const [websiteName, setWebsiteName] = useState('Template 10');
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      apiFetch("/website").then((d) => {
        ...
        setWebsiteName(w.name || 'Template 10');
      ...
      <header className="sticky top-0 z-40" style={{ background: "#111111", borderBottom: "1px solid rgba(224,140,46,0.15)" }}>
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-sm" style={{ color: "#F5F0E8" }}>{websiteName} — Admin Panel</span>
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="flex gap-1 rounded-xl p-1 mb-6 w-fit" style={{ background: "rgba(255,255,255,0.04)" }}>
          {TABS.map((tab) => (
            <button key={tab} type="button" onClick={() => setActiveTab(tab)}
              ...
              style={activeTab === tab
                ? { background: "#E08C2E", color: "#0D0D0D" }
        ...
        {activeTab === "pages" && (
          <div className="space-y-3">
            {/* Quick Visibility Controls section hidden as requested
            ...
            */}
            ...
            {pages.map((page) => (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${page.status === "PUBLISHED" ? "bg-green-500/20 text-green-400" : "bg-yellow-500/20 text-yellow-400"}`}>
                    {page.status}
                  </span>
                  <svg className="w-4 h-4" style={{ color: "rgba(224,140,46,0.4)" }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {activeTab === "footer" && (
          <div className="rounded-xl border p-6 space-y-5" style={{ background: "#111111", borderColor: "rgba(224,140,46,0.15)" }}>
            ...
            <div className="space-y-4">
              <div>
                <label className={lbl} style={lblStyle}>Footer Description</label>
                <textarea rows={3} value={footerForm.description} onChange={(e) => setFooterForm((f) => ({ ...f, description: e.target.value }))} placeholder="Brief description shown in the footer..." className={`${inp} resize-vertical`} style={inpStyle} />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div><label className={lbl} style={lblStyle}>Email</label><input type="email" value={footerForm.email} onChange={(e) => setFooterForm((f) => ({ ...f, email: e.target.value }))} placeholder="info@abc.in" className={inp} style={inpStyle} /></div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-1">
                  {([
                    ...
                    <div key={key}>
                      ...
                      <input type="url" value={(footerForm as any)[key]} onChange={(e) => setFooterForm((f) => ({ ...f, [key]: e.target.value }))} placeholder={ph} className={inp} style={inpStyle} />
                    </div>
                  ))}
                </div>
              </div>
              <div>
                <label className={lbl} style={lblStyle}>Copyright Text</label>
                <input type="text" value={footerForm.copyright} onChange={(e) => setFooterForm((f) => ({ ...f, copyright: e.target.value }))} placeholder="© 2026 abc & Associates. All rights reserved." className={inp} style={inpStyle} />
              </div>
            </div>
            <button type="button" onClick={handleFooterSave} disabled={footerSaving}
              className="flex items-center gap-2 text-sm px-5 py-2.5 rounded-lg disabled:opacity-50 transition-all"
              style={{ background: "#E08C2E", color: "#0D0D0D", fontWeight: 600 }}
            >
        ...
        {activeTab === "contact" && (
          <div className="rounded-xl border p-6 space-y-6" style={{ background: "#111111", borderColor: "rgba(224,140,46,0.15)" }}>
            ...
            <div className="space-y-3 pb-5 border-b" style={{ borderColor: "rgba(224,140,46,0.1)" }}>
              ...
              <div><label className={lbl} style={lblStyle}>Page Heading (H1)</label><input type="text" value={contactForm.heroTitle} onChange={(e) => setContactForm((f) => ({ ...f, heroTitle: e.target.value }))} placeholder="e.g. Contact abc & Associates" className={inp} style={inpStyle} /></div>
            ...
            <div className="space-y-3 pb-5 border-b" style={{ borderColor: "rgba(224,140,46,0.1)" }}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div><label className={lbl} style={lblStyle}>Phone</label><input type="text" value={contactForm.phone} onChange={(e) => setContactForm((f) => ({ ...f, phone: e.target.value }))} placeholder="+91 XX XXXX XXXX" className={inp} style={inpStyle} /></div>
                ...
                <div><label className={lbl} style={lblStyle}>Email</label><input type="email" value={contactForm.email} onChange={(e) => setContactForm((f) => ({ ...f, email: e.target.value }))} placeholder="info@abc.in" className={inp} style={inpStyle} /></div>
                ...
                <div className="sm:col-span-2"><label className={lbl} style={lblStyle}>Address</label><input type="text" value={contactForm.address} onChange={(e) => setContactForm((f) => ({ ...f, address: e.target.value }))} placeholder="Complete business address" className={inp} style={inpStyle} /></div>
                <div className="sm:col-span-2"><label className={lbl} style={lblStyle}>Working Hours</label><input type="text" value={contactForm.workingHours} onChange={(e) => setContactForm((f) => ({ ...f, workingHours: e.target.value }))} placeholder="Mon - Fri: 9:30 AM - 6:30 PM" className={inp} style={inpStyle} /></div>
              </div>
            </div>
            <div className="space-y-3">
              <p className="text-xs font-semibold uppercase tracking-wide" style={{ color: "rgba(245,240,232,0.4)" }}>Map <span style={{ color: "rgba(245,240,232,0.25)", fontWeight: 400, textTransform: "none" }}>(optional)</span></p>
              <div>
                <label className={lbl} style={lblStyle}>Google Maps Embed URL</label>
                <input type="url" value={contactForm.mapUrl} onChange={(e) => setContactForm((f) => ({ ...f, mapUrl: e.target.value }))} placeholder="https://maps.google.com/maps?..." className={inp} style={inpStyle} />
                <p className="text-xs mt-1" style={{ color: "rgba(245,240,232,0.25)" }}>Google Maps → Share → Embed a map → copy the src URL</p>
              </div>
            </div>
            <button type="button" onClick={handleContactSave} disabled={contactSaving}
              className="flex items-center gap-2 text-sm px-5 py-2.5 rounded-lg disabled:opacity-50 transition-all"
              style={{ background: "#E08C2E", color: "#0D0D0D", fontWeight: 600 }}
            >
        ...
        {/* Settings Tab hidden as requested
        {activeTab === "settings" && (
          <div className="rounded-xl border p-6 space-y-6" style={{ background: "#111111", borderColor: "rgba(224,140,46,0.15)" }}>
            <div>
              <h2 className="text-base font-semibold mb-1" style={{ color: "#F5F0E8" }}>Site Settings</h2>
              <p className="text-xs" style={{ color: "rgba(245,240,232,0.35)" }}>Control the live status and admin access for this website.</p>
            </div>
            {[
              { key: "isActive" as const, label: "Website Live", on: "The website is currently live and visible to visitors.", off: "The website is offline. Visitors will see an offline page." },
              { key: "isAdminEnabled" as const, label: "Admin Route Active", on: "The /admin route is accessible. You can log in and manage content.", off: "The /admin route is locked. No one can access the admin panel." },
            ].map(({ key, label, on, off }) => (
              <div key={key} className="flex items-center justify-between p-4 rounded-xl" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(224,140,46,0.1)" }}>
                <div>
                  <p className="text-sm font-medium" style={{ color: "#F5F0E8" }}>{label}</p>
                  <p className="text-xs mt-0.5" style={{ color: "rgba(245,240,232,0.4)" }}>{siteSettings[key] ? on : off}</p>
                </div>
                <button type="button" onClick={() => setSiteSettings((s) => ({ ...s, [key]: !s[key] }))}
                  className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none`}
                  style={{ background: siteSettings[key] ? "#22c55e" : "rgba(255,255,255,0.15)" }}
                >
                  <span className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${siteSettings[key] ? "translate-x-6" : "translate-x-1"}`} />
                </button>
              </div>
            ))}

            {!siteSettings.isAdminEnabled && (
              <div className="rounded-lg p-3 text-xs" style={{ background: "rgba(234,179,8,0.1)", border: "1px solid rgba(234,179,8,0.25)", color: "rgba(234,179,8,0.8)" }}>
            ...
            <button type="button" onClick={handleSettingsSave} disabled={settingsSaving}
              ...
              style={{ background: "#E08C2E", color: "#0D0D0D", fontWeight: 600 }}
            >
              {settingsSaving ? <span className="animate-spin rounded-full h-4 w-4 border-b-2 border-current" /> : <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>}
              Save Settings
            </button>
          </div>
        )}
          ...
          <div className="w-full max-w-sm p-6 rounded-2xl" style={{ background: "#111111", border: "1px solid rgba(224,140,46,0.2)" }}>
            ...
            <div className="flex gap-2 mt-5">
              <button type="button" onClick={handleChangePassword} disabled={pwSaving}
                className="flex-1 text-sm py-2 rounded-lg disabled:opacity-50 transition-all"
                style={{ background: "#E08C2E", color: "#0D0D0D", fontWeight: 600 }}
