...
export function UpdatesTicker({ posts }: Props) {
  ...
  const tickerBg = "bg-primary border border-border shadow-xl rounded-xl overflow-hidden"
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-center gap-2 py-3 px-3 bg-primary/10 border-b border-border flex-shrink-0">
        ...
        <h3 className="text-xs font-bold tracking-widest text-white uppercase select-none">{`What's New`}</h3>
        <span className="w-2 h-2 rounded-full bg-primary text-white animate-pulse" aria-hidden="true" />
      </div>
      ...
      <div className="border-t border-border px-3 py-3 flex-shrink-0 bg-muted/10">
        ...
        {!query && (
          <button
            ...
            className={`mt-2 w-full text-[10px] font-semibold tracking-wider uppercase py-1.5 rounded-md transition-colors ${paused ? 'bg-primary text-white' : 'bg-muted/10 text-white hover:bg-muted/50'}`}
          >
            {paused ? '▶ Resume' : '⏸ Pause'}
          </button>
        )}
