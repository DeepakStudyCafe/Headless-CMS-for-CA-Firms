...
import { OPEN_POSITIONS, CAREER_BENEFITS } from "@/lib/constants";
...
const Career = () => {
  ...
  const benefitsData = careerSection?.textContent?.benefits || CAREER_BENEFITS;
  const positionsData = careerSection?.textContent?.positions || OPEN_POSITIONS;
    ...
    <div className="min-h-screen bg-paper flex flex-col">
      ...
      <section className="relative bg-charcoal py-24 md:py-32 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center px-6">
          ...
          >
            {heroSection?.textContent?.heading || "Build Your Career"}
          </motion.h1>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            {heroSection?.textContent?.subheading || "Shape the future of finance with a team that values growth, integrity, and excellence."}
          </motion.p>
        </div>
      </section>
      ...
      <section className="py-20 md:py-28 px-6 lg:px-8 bg-paper relative">
        ...
        <div className="relative z-10 max-w-7xl mx-auto">
          <SectionHeading label="Life at Our Firm" title="Our Culture" />
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            ...
            >
              <p className="text-charcoal/80 font-sans text-base leading-relaxed mb-5">
                We believe that great work comes from a great workplace. Our culture is built on mutual respect, continuous learning, and the freedom to innovate. Every team member, from articleship trainees to senior partners, is encouraged to share ideas and take ownership.
              </p>
              <p className="text-charcoal/80 font-sans text-base leading-relaxed mb-5">
                With a flat hierarchy and open-door policy, we foster an environment where questions are welcomed, mentorship is natural, and professional growth is not just encouraged — it is actively invested in.
              </p>
              <p className="text-charcoal/80 font-sans text-base leading-relaxed">
                Beyond work, we organize regular team outings, knowledge-sharing sessions, and celebrate milestones together. When you join us, you join a family of professionals who are passionate about making a difference in the world of finance.
              </p>
            </motion.div>
            <motion.div
              initial={{ opacity: 0, x: 50 }}
      ...
      <section className="py-20 md:py-28 px-6 lg:px-8 bg-white relative">
        ...
        <div className="relative z-10 max-w-7xl mx-auto">
          <SectionHeading label="Why Join Us" title="Benefits & Perks" />
      ...
      <section className="py-20 md:py-28 px-6 lg:px-8 bg-paper relative">
        ...
        <div className="relative z-10 max-w-5xl mx-auto">
          ...
          <div className="space-y-6">
            {positionsData.map((position: any, i: number) => (
              <motion.div
                key={position.id}
                ...
                className="bg-white p-6 md:p-8 rounded-2xl border-l-4 border-gold shadow-sm hover:shadow-md transition-shadow"
              >
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                  <div className="flex-1">
                    <h3 className="font-display text-xl text-charcoal mb-2">{position.title}</h3>
                    <div className="flex flex-wrap items-center gap-2 mb-3">
                      <Badge variant="outline" className="border-gold/30 text-gold font-sans text-xs">{position.department}</Badge>
                      <Badge variant="outline" className="border-charcoal/20 text-charcoal/60 font-sans text-xs">{position.type}</Badge>
                      <span className="flex items-center gap-1 text-warm-gray text-xs font-sans">
                        <MapPin className="w-3 h-3" /> {position.location}
                      </span>
                    </div>
                    <p className="text-warm-gray font-sans text-sm leading-relaxed mb-3">{position.description}</p>
                    <p className="text-charcoal/60 font-sans text-xs"><span className="font-medium">Experience:</span> {position.experience}</p>
                  </div>
                  <button
                    onClick={scrollToForm}
                    className="shimmer-btn px-6 py-2.5 text-charcoal font-sans text-xs uppercase tracking-widest font-bold flex-shrink-0 self-start"
                  >
                    Apply Now
                  </button>
                </div>
      ...
      <section ref={formRef} className="py-20 md:py-28 px-6 lg:px-8 bg-charcoal relative">
        ...
        <div className="relative z-10 max-w-3xl mx-auto">
          <SectionHeading label="Apply Now" title="Start Your Journey" light />
          <motion.div
