        ...
        <div className="flex items-center justify-between">
          <Link href="/" className="flex items-center space-x-3">
            ...
            <span className="text-2xl font-bold text-primary-700">
              {websiteData?.name || 'Patel Consulting'}
            </span>
          </Link>
            ...
            {navigation.map((item) => {
              ...
              if (item.hasDropdown) {
                return (
                  ...
                  >
                    <button
                      className={`text-sm font-medium transition-colors hover:text-primary-600 flex items-center gap-1 ${
                        isActive ? 'text-primary-600' : 'text-accent-700'
                      }`}
                    >
                      {item.name}
            ...
            {navigation.map((item) => {
                ...
                return (
                  <div key={item.name}>
                    <button
                      ...
                      className="flex items-center justify-between w-full py-2 text-accent-700 hover:text-primary-600"
                    >
                      {item.name}
              ...
              return (
                <Link
                  key={item.name}
                  href={item.href}
                  className="block py-2 text-accent-700 hover:text-primary-600"
                  onClick={() => setMobileMenuOpen(false)}
                >
                  {item.name}
                </Link>
