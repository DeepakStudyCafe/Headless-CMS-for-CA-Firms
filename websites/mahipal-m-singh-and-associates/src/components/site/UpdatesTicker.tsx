...
export function UpdatesTicker({ posts }: Props) {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex items-center justify-center gap-2 py-3 px-3 bg-primary/10 border-b border-border flex-shrink-0">
      ...
      >
        ...
        {!searching && (isSearchMode ? displayPosts : [...displayPosts, ...displayPosts]).map((post, idx) => (
          <div key={`${post.id}-${idx}`} role="listitem"
            ...
            <Link
              to={`/post/${post.slug}`}
              className="text-xs text-foreground/80 hover:text-primary font-medium leading-snug transition-colors focus:outline-none focus:text-primary line-clamp-3 block decoration-transparent"
      ...
      <div className="border-t border-border px-3 py-3 flex-shrink-0 bg-muted/10">
        ...
        {!query && (
          <button
            ...
            className={`mt-2 w-full text-[10px] font-semibold tracking-wider uppercase py-1.5 rounded-md transition-colors ${paused ? 'bg-primary/20 text-primary' : 'text-muted-foreground hover:text-foreground hover:bg-muted/50'}`}
          >
            {paused ? '▶ Resume' : '⏸ Pause'}
          </button>
        )}
