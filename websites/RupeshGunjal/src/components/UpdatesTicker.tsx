...
export function UpdatesTicker({ posts }: Props) {
  ...
  const tickerBg = " border border-border shadow-xl rounded-xl overflow-hidden"

  return (
    <div
      className={`hidden lg:flex flex-col ${tickerBg}`}
      style={{ height: '580px', width: '110%' }}
      aria-label="Latest Updates"
    >
      {/* Header */}
      <div className="flex items-center justify-center gap-2 py-3 px-3 bg-accent/10 border-b border-border flex-shrink-0">
        <span className="w-2 h-2 rounded-full bg-gray-600 animate-pulse" aria-hidden="true" />
        <h3 className="text-xs font-bold tracking-widest text-gray-600 uppercase select-none">{`What's New`}</h3>
