...
import {
  ...
  INDUSTRIES,
  SERVICE_DETAILS
} from './abhinav-j-and-co-data'

const prisma = new PrismaClient()

async function main() {
  console.log('Seeding Abhinav J & Company website data...')

  const abhinavJAndCo = await prisma.website.upsert({
    where: { slug: 'abhinav-j-and-company' },
    update: {
      name: 'Abhinav J & Company',
      domain: 'https://abhinavjandco.in/',
      logo: 'https://api.digitechai.in/uploads/logo.png',
      phone: '+91 9717302777',
      email: 'abhinavjandco@gmail.com',
      address: 'D-151 A Street No.8 Laxmi Nagar New Delhi-110092',
      ...
      themeConfig: {
        ...
        footerContent: {
           description: "Abhinav J & Company provides comprehensive financial and advisory services to help businesses grow securely, ensuring compliance, strategic planning, and long-term financial stability with personalized expert guidance.",
           copyright: "© 2026 Abhinav J & Company All rights reserved.",
        }
      },
    },
    create: {
      name: 'Abhinav J & Company',
      slug: 'abhinav-j-and-company',
      domain: 'https://abhinavjandco.in/',
      logo: 'https://api.digitechai.in/uploads/logo.png',
      phone: '+91 9717302777',
      email: 'abhinavjandco@gmail.com',
      ...
      themeConfig: {
        ...
        footerContent: {
           description: "Abhinav J & Company provides comprehensive financial and advisory services to help businesses grow securely.",
           copyright: "© 2026 Abhinav J & Company All rights reserved.",
        }
      },
    },
  })
  console.log('✅ Abhinav J & Company (Abhinav J & Company) website created')

  await prisma.section.deleteMany({
    where: { page: { websiteId: abhinavJAndCo.id } }
  })
  await prisma.page.deleteMany({
    where: { websiteId: abhinavJAndCo.id }
  })

  // Home Page
  await prisma.page.create({
    data: {
      websiteId: abhinavJAndCo.id,
      slug: 'home',
      title: 'Home - Abhinav J & Company',
      status: 'PUBLISHED',
  ...
  await prisma.page.create({
    data: {
      websiteId: abhinavJAndCo.id,
      slug: 'about',
      title: 'About Us - Abhinav J & Company',
      status: 'PUBLISHED',
      publishedAt: new Date(),
      sections: {
        create: [
          {
            ...
            textContent: {
              heading: 'About Abhinav J & Company',
  ...
  await prisma.page.create({
    data: {
      websiteId: abhinavJAndCo.id,
      slug: 'services',
      title: 'Services - Abhinav J & Company',
      status: 'PUBLISHED',
      publishedAt: new Date(),
      sections: {
        create: [
  ...
  for (const svc of t5services) {
    ...
    await prisma.page.create({
      data: {
        websiteId: abhinavJAndCo.id,
        slug: svc.slug,
        title: `${svc.title} - Abhinav J & Company`,
        status: 'PUBLISHED',
        publishedAt: new Date(),
        sections: {
          create: [
  ...
  await prisma.page.create({
    data: {
      websiteId: abhinavJAndCo.id,
      slug: 'team',
      title: 'Meet Our Experts - Abhinav J & Company',
      status: 'PUBLISHED',
      publishedAt: new Date(),
      sections: {
        create: [
  ...
  await prisma.page.create({
    data: {
      websiteId: abhinavJAndCo.id,
      slug: 'gallery',
      title: 'Our Gallery - Abhinav J & Company',
      status: 'PUBLISHED',
      publishedAt: new Date(),
      sections: {
        create: [
  ...
  await prisma.page.create({
    data: {
      websiteId: abhinavJAndCo.id,
      slug: 'contact',
      title: 'Contact Us - Abhinav J & Company',
      status: 'PUBLISHED',
      publishedAt: new Date(),
      sections: {
        create: [
  ...
  await prisma.page.create({
    data: {
      websiteId: abhinavJAndCo.id,
      slug: 'career',
      title: 'Join Our Team - Abhinav J & Company',
      status: 'PUBLISHED',
      publishedAt: new Date(),
      sections: {
        create: [
  ...
  await prisma.page.create({
    data: {
      websiteId: abhinavJAndCo.id,
      slug: 'query',
      title: 'Submit a Query - Abhinav J & Company',
      status: 'PUBLISHED',
      publishedAt: new Date(),
      sections: {
        create: [
  ...
  console.log('  ✅ Abhinav J & Company: Pages created')

  // ─── Seed SiteAdmin credentials for /admin route ─────────────────────────────
  const bcrypt = await import('bcryptjs')
  const siteAdminEmail = 'admin@abhinavjandco.in'
  ...
  await (prisma as any).siteAdmin.upsert({
    ...
    create: { websiteId: abhinavJAndCo.id, email: siteAdminEmail, passwordHash },
  })
  console.log(`  ✅ Abhinav J & Company: SiteAdmin credentials set — ${siteAdminEmail} / ${siteAdminPassword}`)

  console.log('\n🎉 Database seed for Abhinav J & Company completed successfully!')

}

main()
  .catch((e) => {
    console.error('Error seeding Abhinav J & Company data:', e)
    process.exit(1)
  })
  .finally(async () => {
