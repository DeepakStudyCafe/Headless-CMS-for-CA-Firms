      ...
      <div className="container-wide mx-auto section-padding !pb-8">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
          ...
          <div className="col-span-2 lg:col-span-1">
            <div className="flex items-center ">
              <div className="w-40 md:w-52 rounded-lg overflow-hidden bg-transparent flex items-center justify-center mb-2">
                <img
                  src="/logo.png"
          ...
          <div className="col-span-2 lg:col-span-1">
            ...
            <div className="space-y-3">
              <div className="flex items-start gap-3 text-sm" style={{ color: 'hsl(var(--secondary-foreground) / 0.7)' }}>
                ...
                <span>{website?.address || "Address loading..."}</span>
              </div>
              <a href={`tel:${website?.phone || ''}`} className="flex items-center gap-3 text-sm" style={{ color: 'hsl(var(--secondary-foreground) / 0.7)' }}>
                <Phone size={16} /> {website?.phone || '+91 Contact loading...'}
              </a>
              <a href={`mailto:${website?.email || ''}`} className="flex items-center gap-3 text-sm" style={{ color: 'hsl(var(--secondary-foreground) / 0.7)' }}>
                <Mail size={16} /> {website?.email || 'Email loading...'}
              </a>
            </div>
          </div>
        </div>
        ...
        <div className="border-t pt-6 flex flex-col md:flex-row items-center justify-between gap-4" style={{ borderColor: 'hsl(var(--secondary-foreground) / 0.15)' }}>
          <p className="text-sm" style={{ color: 'hsl(var(--secondary-foreground) / 0.5)' }}>
            © {new Date().getFullYear()} D U & Associates. All rights reserved.
          </p>
