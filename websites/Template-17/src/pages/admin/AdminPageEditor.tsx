...
export default function AdminPageEditor({ id }: { id?: string }) {
            ...
            <div className="p-5 space-y-4">
              ...
              {Array.isArray(section.textContent?.features) && (
                <div>
                  ...
                  <div className="space-y-2 mt-1">
                    {section.textContent.features.map((feat: any, fi: number) => (
                      <div key={fi} className="flex gap-2 items-center bg-gray-50 rounded-lg p-3">
                        <div className="flex-1">
                          <label className={lbl}>Feature Title</label>
                          <input 
                            value={feat.title !== undefined ? feat.title : feat} 
                            onChange={(e) => { 
                              const s = [...section.textContent.features]; 
                              s[fi] = typeof feat === 'object' ? { ...feat, title: e.target.value } : e.target.value; 
                              updateSection(section.id, "textContent", { features: s }); 
                            }} 
                            className={inp} 
                            placeholder="Feature..." 
                          />
                        </div>
                        <button onClick={() => { const s = section.textContent.features.filter((_: any, i: number) => i !== fi); updateSection(section.id, "textContent", { features: s }); }} className="text-red-400 hover:text-red-600 mt-4 text-xs">✕</button>
                      </div>
                    ))}
                    <button onClick={() => { const s = [...section.textContent.features, { title: "" }]; updateSection(section.id, "textContent", { features: s }); }} className="text-xs text-[#2A3B36] hover:underline">+ Add Feature</button>
                  </div>
                </div>
              )}
                  ...
                  <div className="space-y-3 mt-1">
                    {section.textContent.items.map((item: any, ii: number) => (
                      <div key={ii} className="border border-gray-200 rounded-xl p-4 bg-gray-50 space-y-3">
                        <div className="flex justify-between items-center">
                          <span className="text-xs font-semibold text-gray-500">{section.type === "team" ? (item.name || `Member ${ii + 1}`) : (item.title || item.question || `Item ${ii + 1}`)}</span>
                        ...
                        {item.desc !== undefined && <div><label className={lbl}>Description (desc)</label><textarea rows={2} value={item.desc || ""} onChange={(e) => { const its = [...section.textContent.items]; its[ii] = { ...item, desc: e.target.value }; updateSection(section.id, "textContent", { items: its }); }} className={`${inp} resize-vertical`} placeholder="Description..." /></div>}
                        {item.text !== undefined && <div><label className={lbl}>Text / Review</label><textarea rows={3} value={item.text || ""} onChange={(e) => { const its = [...section.textContent.items]; its[ii] = { ...item, text: e.target.value }; updateSection(section.id, "textContent", { items: its }); }} className={`${inp} resize-vertical`} placeholder="Review text..." /></div>}
                        {item.question !== undefined && <div><label className={lbl}>Question</label><input value={item.question || ""} onChange={(e) => { const its = [...section.textContent.items]; its[ii] = { ...item, question: e.target.value }; updateSection(section.id, "textContent", { items: its }); }} className={inp} placeholder="Question..." /></div>}
                        {item.answer !== undefined && <div><label className={lbl}>Answer</label><textarea rows={2} value={item.answer || ""} onChange={(e) => { const its = [...section.textContent.items]; its[ii] = { ...item, answer: e.target.value }; updateSection(section.id, "textContent", { items: its }); }} className={`${inp} resize-vertical`} placeholder="Answer..." /></div>}
                        {item.year !== undefined && <div><label className={lbl}>Year</label><input value={item.year || ""} onChange={(e) => { const its = [...section.textContent.items]; its[ii] = { ...item, year: e.target.value }; updateSection(section.id, "textContent", { items: its }); }} className={inp} placeholder="2020" /></div>}
                    ...
                    <button
                      onClick={() => {
                        const newItem = section.type === "team"
                          ? { name: "", designation: "", qualifications: "", bio: "", img: "", linkedin: "#" }
                          : section.type === "gallery"
                          ? { src: "", alt: "", category: "Office", title: "" }
                          : section.type === "faq"
                          ? { question: "", answer: "" }
                          : section.type === "testimonials" || section.type === "testimonial"
