...
async function main() {
  console.log('Starting seed process for Vineet Taral & Associates...');
  
  try {
    await prisma.$transaction(async (tx) => {
        // Find existing website or create
        let website = await tx.website.findUnique({
          where: { slug: 'vineet-taral-and-associates' }
        });
        
        if (!website) {
          website = await tx.website.create({
            data: {
              name: 'Vineet Taral & Associates Chartered Accountant',
              slug: 'vineet-taral-and-associates',
              isActive: true,
            }
          });
          console.log('Created website: Vineet Taral & Associates');
        } else {
          console.log('Website Vineet Taral & Associates already exists');
        }
    ...
    console.log('Successfully completed full deep seeding for Vineet Taral & Associates!');

  } catch (err) {
    console.error('Error seeding vineet-taral-and-associates:', err);
  } finally {
