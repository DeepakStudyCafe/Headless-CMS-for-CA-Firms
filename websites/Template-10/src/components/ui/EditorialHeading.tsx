  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <motion.span
        ...
        className={`font-label text-xs tracking-[3px] mb-3 block ${light ? "text-secondary" : "text-secondary"}`}
      >
        {label}
      </motion.span>

      {/* Heading with letter-by-letter 3D flip */}
      <h2 className={`font-display ${headingSize} font-semibold ${light ? "text-surface" : "text-on-surface"} leading-[1.1]`}>
      ...
      <motion.div
        ...
        className={`h-[1px] w-16 mt-4 origin-left ${light ? "bg-secondary/40" : "bg-secondary"}`}
      />
    </div>
  );
};
