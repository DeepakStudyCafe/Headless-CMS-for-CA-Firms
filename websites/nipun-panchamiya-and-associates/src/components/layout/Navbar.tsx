...
const Navbar = ({ websiteData, disableSticky = false }: { websiteData?: any, disableSticky?: boolean }) => {
  ...
  return (
    <>
      <motion.nav
        ...
        className={`${disableSticky ? "absolute bg-ink" : "fixed"} top-0 left-0 right-0 z-50 transition-all duration-500 ${
          scrolled && !disableSticky
            ? "bg-ink/[0.92] backdrop-blur-xl border-b border-gold/25 py-4 px-6 lg:px-16"
            : disableSticky ? "py-7 px-6 lg:px-16" : "bg-transparent py-7 px-6 lg:px-16"
        }`}
      >
