...
import { UpdatesTicker, TickerPost } from '@/components/UpdatesTicker';
import { Link } from '@tanstack/react-router';

export function Services({ data, tickerPosts = [] }: { data: any, tickerPosts?: TickerPost[] }) {
    ...
    <section id='services' className='relative bg-section py-28 md:py-36 px-6 overflow-hidden'>
      ...
      <div className='relative max-w-7xl mx-auto'>
        ...
        <div className='flex flex-col lg:flex-row gap-8 items-start'>
          <div className='flex-1 w-full grid md:grid-cols-6 gap-5 md:gap-6 auto-rows-fr'>
            {items.map((s: any, i: number) => {
              const Icon = (Icons as any)[s.icon] || Icons.FileText;
              return (
              <Link to={`/services/${s.title.toLowerCase().replace(/\s+/g, '-')}`} key={i} className={`block ${s.featured ? 'md:col-span-2' : 'md:col-span-2'}`}>
                <motion.article
          ...
          {tickerPosts && tickerPosts.length > 0 && (
