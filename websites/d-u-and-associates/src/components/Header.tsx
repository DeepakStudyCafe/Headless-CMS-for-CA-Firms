...
const Header = () => {
  ...
  useEffect(() => {
    // Scroll listener removed to keep before-scroll state only
  }, []);
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 bg-white shadow-sm`}>
      {/* Top bar */}
      <div className={`transition-all duration-300 h-auto opacity-100`} style={{ backgroundColor: 'hsl(var(--primary))' }}>
          ...
          <div className="w-full flex items-center justify-between">
            <div className="flex items-center gap-3 min-w-0">
              <a href="tel:+911234567890" className="flex items-center gap-1 hover:opacity-80 transition-opacity whitespace-nowrap">
                <Phone size={14} /> <span className="ml-1">+91 9296293692</span>
              </a>
      ...
      <nav className="container-wide mx-auto px-4 flex items-center justify-between h-16 md:h-18">
        ...
        <div className="hidden lg:flex items-center gap-1">
          {navigation.map((item) => (
            <div key={item.name} className="relative" onMouseEnter={() => item.hasDropdown && setDropdownOpen(true)} onMouseLeave={() => item.hasDropdown && setDropdownOpen(false)}>
              <Link
                to={item.href}
                className={`px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 flex items-center gap-1 text-muted-foreground hover:text-primary ${location.pathname === item.href ? 'text-primary' : ''}`}
              >
                {item.name}
        ...
        {/* Mobile toggle */}
        <button onClick={() => setMobileOpen(!mobileOpen)} className={`lg:hidden p-2 rounded-lg text-foreground`}>
