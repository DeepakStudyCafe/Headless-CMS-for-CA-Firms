      ...
      <main className="max-w-5xl mx-auto px-4 py-6 space-y-5">
        ...
        {sections.map((section, idx) => (
          <SectionEditor
            ...
            onSetTC={(patch: Record<string, any>) => setTC(section.id, patch)}
            onSetItemField={(arrayKey: string, itemIdx: number, field: string, val: string) => setItemField(section.id, arrayKey, itemIdx, field, val)}
            onItemImageUpload={(arrayKey: string, itemIdx: number, file: File) => handleItemImageUpload(section.id, arrayKey, itemIdx, file)}
          />
        ))}
        {sections.length > 0 && (
...
// ---------------- SectionEditor component (kept local) ----------------
function SectionEditor({ section, idx, onImageUpload, onRemoveImage, onSetTC, onSetItemField, onItemImageUpload }: any) {
          ...
          <div className="space-y-2">
            {(tc.stats as { value: string; label: string }[]).map((stat, si) => (
              <div key={si} className="grid grid-cols-2 gap-2">
                <div>
                  <input value={stat.value || ''} onChange={(e) => onSetItemField('stats', si, 'value', e.target.value)} className={inp} />
                </div>
                <div>
                  <input value={stat.label || ''} onChange={(e) => onSetItemField('stats', si, 'label', e.target.value)} className={inp} />
                </div>
              </div>
            ))}
          </div>
