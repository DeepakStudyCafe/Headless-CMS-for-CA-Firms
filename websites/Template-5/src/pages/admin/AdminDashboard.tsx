...
const inp =
  "w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#2A3B36]";
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-40">
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-gray-800 text-sm">Vanguard &amp; Co. — Admin Panel</span>
            ...
            {!siteSettings.isAdminEnabled && (
              <span className="text-xs bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full font-medium">Admin Locked</span>
            )}
          </div>
          <div className="flex items-center gap-2">
            <button
            ...
            <button
              ...
              className="text-xs bg-[#2A3B36] text-white px-3 py-1.5 rounded-lg hover:bg-[#1F2C28] transition-colors"
            >
              Logout
            </button>
          </div>
          ...
          <div className="space-y-3">
            ...
            {pages.map((page) => (
              <button
                ...
                className="w-full bg-white rounded-xl border border-gray-200 px-5 py-4 flex items-center justify-between hover:bg-gray-50 hover:border-[#2A3B36]/30 transition-all text-left group"
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-[#2A3B36]/5 rounded-lg flex items-center justify-center group-hover:bg-[#2A3B36]/10 transition-colors">
                    <svg className="w-4 h-4 text-[#2A3B36]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <svg className="w-4 h-4 text-gray-400 group-hover:text-[#2A3B36] transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div><label className={lbl}>Email</label><input type="email" autoComplete="off" value={footerForm.email} onChange={(e) => setFooterForm((f) => ({ ...f, email: e.target.value }))} placeholder="info@vanguardca.in" className={inp} /></div>
            ...
            <button type="button" onClick={handleFooterSave} disabled={footerSaving} className="flex items-center gap-2 bg-[#2A3B36] text-white text-sm px-5 py-2.5 rounded-lg hover:bg-[#1F2C28] disabled:opacity-50 transition-colors">
              {footerSaving ? <span className="animate-spin rounded-full h-4 w-4 border-b-2 border-white" /> : <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>}
              Save Footer
            </button>
          </div>
        )}
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-6">
            ...
            <div className="space-y-3 pb-5 border-b border-gray-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div><label className={lbl}>Email</label><input type="email" autoComplete="off" value={contactForm.email} onChange={(e) => setContactForm((f) => ({ ...f, email: e.target.value }))} placeholder="info@vanguardca.in" className={inp} /></div>
                <div><label className={lbl}>Email 2 <span className="text-gray-400 font-normal">(optional)</span></label><input type="email" autoComplete="off" value={contactForm.email2} onChange={(e) => setContactForm((f) => ({ ...f, email2: e.target.value }))} placeholder="support@vanguardca.in" className={inp} /></div>
            ...
            <button type="button" onClick={handleContactSave} disabled={contactSaving} className="flex items-center gap-2 bg-[#2A3B36] text-white text-sm px-5 py-2.5 rounded-lg hover:bg-[#1F2C28] disabled:opacity-50 transition-colors">
        ...
        {activeTab === "settings" && (
          <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-6">
            <div>
              ...
              <p className="text-xs text-gray-400">Control the live status and admin access for this website.</p>
            </div>

            {/* Site Live Toggle */}
            <div className="flex items-center justify-between p-4 rounded-xl border border-gray-100 bg-gray-50">
              <div>
                <p className="text-sm font-medium text-gray-900">Website Live</p>
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${
                  siteSettings.isActive ? "bg-green-500" : "bg-gray-300"
                }`}
              >
                <span
            ...
            <div className="flex items-center justify-between p-4 rounded-xl border border-gray-100 bg-gray-50">
              <div>
                <p className="text-sm font-medium text-gray-900">Admin Route Active</p>
                <p className="text-xs text-gray-500 mt-0.5">
                  {siteSettings.isAdminEnabled
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${
                  siteSettings.isAdminEnabled ? "bg-green-500" : "bg-gray-300"
                }`}
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${
            ...
            <button
              ...
              className="flex items-center gap-2 bg-[#2A3B36] text-white text-sm px-5 py-2.5 rounded-lg hover:bg-[#1F2C28] disabled:opacity-50 transition-colors"
            >
              {settingsSaving ? <span className="animate-spin rounded-full h-4 w-4 border-b-2 border-white" /> : <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>}
              Save Settings
            </button>
          </div>
        )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            <div className="flex gap-2 mt-5">
              <button type="button" onClick={handleChangePassword} disabled={pwSaving} className="flex-1 bg-[#2A3B36] text-white text-sm py-2 rounded-lg hover:bg-[#1F2C28] disabled:opacity-50 transition-colors">
