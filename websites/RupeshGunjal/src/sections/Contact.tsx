...
export function Contact({ data }: { data: any }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Reset floating fields by dispatching event or letting user reload
    } catch {
      setStatus('idle');
        ...
        >
          <div className='relative bg-section p-8 md:p-12 flex flex-col justify-between overflow-hidden'>
            ...
            <div className='relative'>
              <h3 className='font-display text-3xl md:text-4xl text-foreground leading-tight' dangerouslySetInnerHTML={{ __html: content.panelHeading || '' }} />
            ...
            <div className='relative mt-10 space-y-5'>
              {(content.contacts || []).map((c: any, i: number) => {
                const Icon = (Icons as any)[c.icon] || Icons.Check;
                return (
                <div key={i} className='flex items-start gap-4 group'>
                  <div className='h-11 w-11 rounded-xl bg-card border border-border flex items-center justify-center group-hover:border-primary/60 group-hover:bg-gradient-gold transition-all duration-300'>
                    <Icon size={17} className='text-primary group-hover:text-primary-foreground transition-colors' strokeWidth={1.5} />
                  </div>
                  <div>
          ...
          <div className='p-8 md:p-12'>
            <form onSubmit={handleSubmit} className='space-y-5'>
              <div className='grid sm:grid-cols-2 gap-5'>
                <FloatingField label='Full Name' type='text' id='name' name='name' />
                <FloatingField label='Email Address' type='email' id='email' name='email' />
              </div>
              <FloatingField label='Phone Number' type='tel' id='phone' name='phone' />
              <FloatingField label='How can we help?' type='textarea' id='message' name='message' />
              ...
              >
                {status === 'idle' && (
                  <>
                    Send Message
                    <Icons.ArrowUpRight size={16} className='transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5' />
                  </>
                )}
                {status === 'loading' && (
                  <>
                    <Icons.Loader2 size={16} className='animate-spin' /> Sending...
                  </>
                )}
                {status === 'sent' && (
                  <>
                    <Icons.Check size={16} /> Message Sent
                  </>
                )}
              </button>
...
function FloatingField({ label, type, id, name }: { label: string; type: string; id: string; name: string }) {
