...
const TestimonialCard = ({
  ...
  >
    {/* Quote mark */}
    <span className="font-display text-[64px] text-ca-accent-2 leading-[0.8] block mb-2 select-none" style={{ opacity: 0.35 }}>
      "
    </span>

    {/* Quote text */}
    <p className="font-display italic text-[16px] leading-[1.7] text-text-main mb-auto line-clamp-4">
      {testimonial.text}
    </p>

    {/* Divider */}
    <div className="h-[1px] bg-ca-accent-2/20 my-3" />

    {/* Author row */}
    <div className="flex items-center justify-between">
      <div className="flex items-center gap-2.5">
        <div className="w-8 h-8 rounded-full bg-ca-accent-2 flex items-center justify-center text-surface font-label text-[12px] flex-shrink-0">
          {testimonial.initials}
        </div>
        <div>
          <p className="font-body text-[14px] font-semibold text-text-main leading-tight">{testimonial.name}</p>
          <p className="font-body text-[11px] text-text-muted-custom">{testimonial.designation}</p>
        </div>
      </div>
    ...
    >
      ...
      <div className="relative z-10 container mx-auto px-6">
        ...
        <div className="flex items-center justify-center gap-5 mt-8">
          <button
            onClick={prev}
            className="w-9 h-9 rounded-full border border-surface/20 flex items-center justify-center text-surface/60 hover:bg-ca-accent hover:text-surface hover:border-ca-accent transition-all duration-300"
          >
            <ChevronLeft size={16} />
          </button>
            ...
            {TESTIMONIALS.map((_, i) => (
              <button
                ...
                className={`h-2 rounded-full transition-all duration-400 ${
                  i === active ? "w-6 bg-ca-accent-2" : "w-2 bg-surface/25"
                }`}
              />
            ))}
          </div>
