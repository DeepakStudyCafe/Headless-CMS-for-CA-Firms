...
const DEFAULT_SERVICE_LINKS = ['Tax Planning', 'GST Filing', 'Audit Services', 'Company Registration', 'Bookkeeping'];
...
export default function Footer({ websiteData }: { websiteData?: any }) {
  ...
  const address = websiteData?.address || '123 Financial District, New Delhi, 110001';
  const phone = websiteData?.phone || '';
  const email = websiteData?.email || '';
  const footerContent = websiteData?.themeConfig?.footerContent || {};
  const copyright = footerContent.copyright || `© ${new Date().getFullYear()} ${name}`;

  const serviceLinks = (websiteData?.themeConfig?.services || []).map((s: any) => s.title);
  const displayServiceLinks = serviceLinks.length > 0 ? serviceLinks : DEFAULT_SERVICE_LINKS;
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div>
            ...
            <p className="font-body font-light text-[14px] leading-relaxed mb-2" style={{ color: 'rgba(250,248,243,0.45)' }}>
              {footerContent.description || 'Your trusted partner in financial excellence.'}
            </p>
          ...
          <div>
            ...
            {displayServiceLinks.map((link: string) => (
              <a key={link} href="#" data-cursor="button"
                className="group block font-body text-[13px] py-1.5 transition-all duration-200 hover:translate-x-[5px]"
                style={{ color: 'rgba(250,248,243,0.5)' }}
              >
          ...
          <div>
            ...
            <div className="space-y-3">
              {[
                { icon: 'M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z M12 10m-3 0a3 3 0 106 0 3 3 0 00-6 0', text: address },
                phone ? { icon: 'M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z', text: phone } : null,
                email ? { icon: 'M2 4h20v16H2z M22 4l-10 8L2 4', text: email } : null,
