...
const Gallery = () => {
  ...
  const galleryCategories = gallerySection?.textContent?.categories || GALLERY_CATEGORIES;
  const galleryImagesData = gallerySection?.textContent?.items || GALLERY_IMAGES;
    ...
    <div className="min-h-screen bg-paper flex flex-col">
      ...
      <section className="relative bg-charcoal py-24 md:py-32 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center px-6">
          ...
          >
            {heroSection?.textContent?.heading || "Our Gallery"}
          </motion.h1>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            {heroSection?.textContent?.subheading || "Glimpses of our workplace, events, and the people who make it all happen."}
          </motion.p>
        </div>
      </section>
      ...
      <section className="py-20 md:py-28 px-6 lg:px-8 bg-paper relative">
        ...
        <div className="relative z-10 max-w-7xl mx-auto">
          <SectionHeading label="Visual Stories" title="Moments & Milestones" />
      ...
      <section className="bg-charcoal py-16 px-6 lg:px-8 relative border-t-2 border-gold/30">
        ...
        <div className="relative z-10 max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          {[
            ...
            { icon: Users, value: "100+", label: "Seminar Attendees" },
            { icon: MapPin, value: "5", label: "Office Locations" },
            { icon: Camera, value: "20+", label: "Years of Memories" },
          ].map((stat, i) => (
            <motion.div
              key={stat.label}
              initial={{ opacity: 0, y: 20 }}
