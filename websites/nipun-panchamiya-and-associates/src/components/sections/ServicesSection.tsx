...
const ServicesSection = ({ data, posts = [] }: { data?: any, posts?: any[] }) => {
      ...
      <div ref={ref} className="relative max-w-[1200px] mx-auto px-6">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          {posts && posts.length > 0 && (
            <aside className="w-full lg:w-[350px] flex-shrink-0 z-10 relative" aria-label="Latest updates">
...
const ServiceCard = ({
  ...
  return (
    <motion.div
      ...
      className={`relative p-7 border rounded-sm overflow-hidden cursor-pointer transition-all duration-[350ms] group flex flex-col h-full ${
        hovered
    ...
    >
      ...
      <Icon
        size={40}
        className={`mt-2 mb-3 transition-colors duration-300 ${hovered ? "text-gold" : "text-ink"}`}
        strokeWidth={1.2}
      />

      <h3 className={`font-display text-xl font-semibold mb-3 transition-colors duration-300 ${hovered ? "text-cream" : "text-ink"}`}>
        {service.title}
      </h3>
      ...
      <p className={`font-body text-[14px] leading-relaxed transition-colors duration-300 flex-1 ${hovered ? "text-mist" : "text-slate"}`}>
        {service.desc}
      </p>

      <span
        className={`inline-block mt-3 font-body text-sm text-gold transition-all duration-300 ${
