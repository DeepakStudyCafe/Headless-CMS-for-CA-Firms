...
function SectionBlock({ section, websiteData }: { section: any; websiteData?: any }) {
  ...
  const renderSectionTitle = (fallback: string) => (
    <h2 className="font-display text-3xl sm:text-5xl text-on-surface mb-8">{tc.heading || fallback}</h2>
  );
    ...
    return (
      <section className="py-16 bg-surface">
        <div className="container mx-auto px-6">
          <h2 className="font-display text-3xl sm:text-5xl text-on-surface mb-8">{tc.heading || "Services"}</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {tc.items.map((item: any, i: number) => (
              <div key={i} className="bg-white rounded-lg border border-primary/10 p-6">
                <h3 className="font-body text-lg font-semibold text-on-surface">{item.title || item.name}</h3>
                {item.description && <p className="mt-2 text-sm text-on-surface-variant">{item.description}</p>}
              </div>
            ))}
          </div>
        </div>
  ...
  if (section.type === "team" && Array.isArray(tc.items)) {
    return (
      <section className="py-16 bg-surface font-body text-on-surface">
        <div className="container mx-auto px-6">
          <h2 className="font-display text-3xl sm:text-5xl text-on-surface mb-8">{tc.heading || "Our Team"}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {tc.items.map((item: any, i: number) => (
              <div key={i} className="bg-surface rounded-lg border border-primary/10 overflow-hidden">
                ...
                <div className="p-4">
                  <h3 className="font-body text-base font-semibold text-on-surface">{item.name || item.title}</h3>
                  {item.designation && <p className="text-sm text-on-surface-variant">{item.designation}</p>}
                  {item.bio && <p className="text-xs text-on-surface-variant mt-2">{item.bio}</p>}
                </div>
              </div>
            ))}
          </div>
  ...
  if (section.type === "gallery" && Array.isArray(tc.items)) {
    return (
      <section className="py-16 bg-surface font-body text-on-surface">
        <div className="container mx-auto px-6">
          <h2 className="font-display text-3xl sm:text-5xl text-on-surface mb-8">{tc.heading || "Gallery"}</h2>
    ...
    return (
      <section className="py-16 bg-surface font-body text-on-surface">
        <div className="container mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
          <div>
            <h2 className="font-display text-3xl sm:text-5xl text-on-surface">{tc.heading || "Overview"}</h2>
            {tc.description && <p className="mt-4 text-on-surface-variant">{tc.description}</p>}
            {paragraphs.length > 0 && (
              <div className="mt-4 space-y-3">
                {paragraphs.map((p: string, i: number) => (
                  <p key={i} className="text-on-surface-variant text-sm leading-7">{p}</p>
                ))}
              </div>
            )}
            {features.length > 0 && (
              ...
              <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-3">
                {items.map((item: any, i: number) => (
                  <div key={i} className="bg-surface rounded-lg border border-primary/10 p-3">
                    <p className="text-sm font-semibold text-on-surface">{item.title || item.name || `Item ${i + 1}`}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {tc.items.map((item: any, i: number) => (
              <div key={i} className="bg-surface rounded-lg border border-primary/10 p-5">
                <h3 className="font-body text-base font-semibold text-on-surface">{item.title || item.name || `Feature ${i + 1}`}</h3>
                {item.description && <p className="mt-2 text-sm text-on-surface-variant">{item.description}</p>}
              </div>
            ))}
          </div>
          ...
          <div className="relative border-l border-primary/20 pl-6 space-y-6">
            {tc.items.map((item: any, i: number) => (
              <div key={i} className="relative">
                <span className="absolute -left-[31px] top-0 h-3 w-3 rounded-full bg-secondary" />
                <p className="text-xs text-secondary font-semibold">{item.year || item.date || `Year ${i + 1}`}</p>
                <p className="text-base text-on-surface font-semibold">{item.title || item.name}</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {tc.items.map((item: any, i: number) => (
              <div key={i} className="rounded-lg border border-primary/10 p-5">
                <p className="font-body text-base font-semibold text-on-surface">{item.title || item.name || `Certification ${i + 1}`}</p>
                {item.issuer && <p className="text-xs text-secondary mt-1">{item.issuer}</p>}
                {item.description && <p className="text-sm text-on-surface-variant mt-2">{item.description}</p>}
              </div>
            ))}
          </div>
        </div>
  ...
  if (section.type === "stats" && Array.isArray(tc.stats)) {
    return (
      <section className="py-14 bg-primary-container text-surface">
        <div className="container mx-auto px-6 grid grid-cols-2 lg:grid-cols-4 gap-6">
          {tc.stats.map((s: any, i: number) => (
            <div key={i} className="text-center">
              <AnimatedStatValue
                value={`${s.value ?? s.number ?? ""}${s.suffix || ""}`}
                className="font-display text-4xl text-secondary"
              />
  ...
  if (section.type === "testimonials" && Array.isArray(tc.items)) {
    return <Testimonials data={section} />;
  }

  if (section.type === "faqs" && Array.isArray(tc.items)) {
    return (
      <section className="py-16 bg-primary-container text-surface">
        <div className="container mx-auto px-6">
          <h2 className="font-display text-3xl sm:text-5xl mb-8">{tc.heading || "FAQs"}</h2>
          <div className="flex flex-col gap-4 max-w-4xl">
            {tc.items.map((item: any, i: number) => (
              <div key={i} className="bg-surface/5 border border-surface/10 rounded-lg p-5">
                <p className="text-base font-semibold text-surface/90">{item.question || item.q || item.title || item.text}</p>
                {item.answer && <p className="mt-2 text-sm text-surface/70 leading-relaxed">{item.answer}</p>}
                {item.a && <p className="mt-2 text-sm text-surface/70 leading-relaxed">{item.a}</p>}
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 bg-surface font-body text-on-surface">
        <div className="container mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {tc.items.map((item: any, i: number) => (
              <div key={i} className="bg-surface rounded-lg border border-primary/10 p-5">
                <p className="font-body text-base font-semibold text-on-surface">{item.title || item.name || `Plan ${i + 1}`}</p>
                <p className="text-2xl font-display text-secondary mt-2">{item.price || item.value || "Custom"}</p>
                {item.description && <p className="text-sm text-on-surface-variant mt-2">{item.description}</p>}
              </div>
            ))}
          </div>
  ...
  if (section.type === "cta") {
    return (
      <section className="py-16 bg-primary-container text-center">
        <div className="container mx-auto px-6">
  ...
  if (section.type === "process" && Array.isArray(tc.items)) {
    return (
      <section className="py-16 bg-surface font-body text-on-surface">
        <div className="container mx-auto px-6">
          <h2 className="font-display text-3xl sm:text-5xl text-on-surface mb-8">{tc.heading || "Process"}</h2>
          <div className="space-y-4">
            {tc.items.map((p: any, i: number) => (
              <div key={i} className="p-4 rounded-lg border border-primary/15 bg-surface">
                <p className="font-body font-semibold text-on-surface">{p.title || `Step ${i + 1}`}</p>
      ...
      <section className="py-16 bg-surface font-body text-on-surface">
        <div className="container mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-surface rounded-xl border border-primary/10 p-6">
            <h3 className="font-display text-2xl text-on-surface">{heading}</h3>
            <div className="mt-4 space-y-2 text-sm text-on-surface-variant">
              <p><span className="text-on-surface font-semibold">Phone:</span> {info.phone || "-"}</p>
              {info.phone2 && <p><span className="text-on-surface font-semibold">Phone 2:</span> {info.phone2}</p>}
              <p><span className="text-on-surface font-semibold">Email:</span> {info.email || "-"}</p>
              {info.email2 && <p><span className="text-on-surface font-semibold">Email 2:</span> {info.email2}</p>}
              <p><span className="text-on-surface font-semibold">Address:</span> {info.address || "-"}</p>
              <p><span className="text-on-surface font-semibold">Hours:</span> {typeof info.hours === "object" ? Object.values(info.hours).filter(Boolean).join(" | ") : (info.hours || "-")}</p>
              {contactContent.mapUrl && (
                <a href={contactContent.mapUrl} target="_blank" rel="noreferrer" className="inline-block mt-2 text-secondary hover:underline">
                  Open Map
                </a>
              )}
            </div>
          ...
          <div className="bg-surface rounded-xl border border-primary/10 p-6">
            <h3 className="font-display text-2xl text-on-surface">{contactContent.heroSubtitle ? "FAQs & Support" : "FAQs"}</h3>
            {contactContent.heroSubtitle && <p className="text-xs text-on-surface-variant mt-2">{contactContent.heroSubtitle}</p>}
            <div className="mt-4 space-y-3">
              {faqs.slice(0, 5).map((f: any, i: number) => (
                <div key={i}>
                  <p className="text-sm font-semibold text-on-surface">{f.question || f.title}</p>
                  <p className="text-sm text-on-surface-variant">{f.answer || ""}</p>
                </div>
              ))}
            </div>
          </div>
  ...
  if (section.type === "career") {
    ...
    return (
      <section className="py-16 bg-surface font-body text-on-surface">
        <div className="container mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div>
            <h3 className="font-display text-3xl text-on-surface">{tc.benefitsHeading || "Benefits & Perks"}</h3>
            <div className="mt-4 space-y-3">
              {benefits.map((b: any, i: number) => (
                <div key={i} className="bg-surface rounded-lg border border-primary/10 p-4">
                  <p className="font-semibold text-on-surface text-sm">{b.title || b.name || `Benefit ${i + 1}`}</p>
                  {b.description && <p className="text-xs text-on-surface-variant mt-1">{b.description}</p>}
                </div>
              ))}
            </div>
          </div>
          <div>
            <h3 className="font-display text-3xl text-on-surface">{tc.positionsHeading || "Open Positions"}</h3>
            <div className="mt-4 space-y-3">
              {positions.map((p: any, i: number) => (
                <div key={i} className="bg-surface rounded-lg border border-primary/10 p-4">
                  <p className="font-semibold text-on-surface text-sm">{p.title || `Role ${i + 1}`}</p>
                  <p className="text-xs text-secondary mt-1">{p.location || "India"} {p.type ? `• ${p.type}` : ""}</p>
                  {p.description && <p className="text-xs text-on-surface-variant mt-1">{p.description}</p>}
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="py-16 bg-surface font-body text-on-surface">
        <div className="container mx-auto px-6 max-w-3xl">
          <div className="bg-surface rounded-xl border border-primary/10 p-8">
            <h3 className="font-display text-3xl text-on-surface">{tc.formHeading || "Submit Your Query"}</h3>
            {tc.formSubheading && <p className="mt-2 text-sm text-on-surface-variant">{tc.formSubheading}</p>}
            <div className="mt-5 flex flex-wrap gap-2">
              {types.map((t: any, i: number) => (
                <span key={i} className="px-3 py-1 rounded-full border border-primary/15 text-xs text-on-surface bg-surface font-body text-on-surface">{t.label || t.value}</span>
              ))}
            </div>
          </div>
        </div>
  ...
  if (section.type === "industries" && Array.isArray(tc.items)) {
    return (
      <section className="py-16 bg-surface font-body text-on-surface">
        <div className="container mx-auto px-6">
          <h2 className="font-display text-3xl sm:text-5xl text-on-surface mb-8">{tc.heading || "Industries"}</h2>
          <div className="flex flex-wrap gap-3">
            {tc.items.map((ind: any, i: number) => (
              <span key={i} className="px-4 py-2 rounded-full bg-surface border border-primary/15 text-sm text-on-surface">
