    ...
    <div className="pb-20 min-h-screen" style={{ backgroundImage: "url('/about.jpeg')" }}>
      <section className="py-12 md:py-20">
        <div className="container-custom px-4">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center max-w-4xl mx-auto pt-12 md:pt-24">
            <h1 className="text-3xl sm:text-5xl md:text-6xl font-bold text-white mb-4 md:mb-6">About Webcafe</h1>
            <p className="text-base sm:text-xl text-white/80 leading-relaxed">We're passionate about helping Chartered Accountants establish a strong online presence. Our mission is to provide professional, affordable, and effective website solutions tailored specifically for CA firms.</p>
          </motion.div>
        </div>
      </section>
            ...
            {values.map((v, i) => {
              ...
              return (
                <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }} className="text-center">
                  ...
                  <h3 className="text-xl md:text-2xl font-bold text-gray-900 mb-2 md:mb-3">{v.title}</h3>
                  <p className="text-sm md:text-base text-gray-600 leading-relaxed">{v.description}</p>
                </motion.div>
              )
            })}
          </div>
        ...
        <div className="container-custom px-4">
          <div className="max-w-4xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-10 md:mb-12">
              <h2 className="text-2xl md:text-4xl lg:text-5xl font-bold text-gray-900 mb-4 md:mb-6">Our Story</h2>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2 }} className="prose prose-sm md:prose-lg max-w-none">
              <div className="bg-white rounded-2xl shadow-xl p-6 md:p-12">
                <p className="text-gray-600 leading-relaxed mb-4 md:mb-6 text-sm md:text-base">Webcafe was founded in 2026 with a simple yet powerful vision: to help Chartered Accountants establish a professional online presence with full control over their website content through integrated admin panels.</p>
                <p className="text-gray-600 leading-relaxed mb-4 md:mb-6 text-sm md:text-base">Having worked closely with numerous CA firms, we understood their unique needs and challenges. We realized that CA professionals needed not just beautiful websites, but also the ability to manage and update content themselves without technical knowledge.</p>
                <p className="text-gray-600 leading-relaxed text-sm md:text-base">This insight led us to create a collection of <span className="font-bold text-gray-900">Professionally designed templates</span>, each with a built-in admin panel.</p>
              </div>
            </motion.div>
          </div>
        </div>
      ...
      <section className="py-16 md:py-20 bg-gradient-to-br from-primary-600 to-blue-700">
        <div className="container-custom px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
            {stats.map((s, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }} className="text-center">
                <div className="text-3xl md:text-5xl font-bold text-white mb-1 md:mb-2">{s.number}</div>
                <div className="text-xs md:text-base text-white/90">{s.label}</div>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 md:py-20 bg-white">
        <div className="container-custom px-4">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-12 md:mb-16">
            <h2 className="text-2xl md:text-4xl lg:text-5xl font-bold text-gray-900 mb-4 md:mb-6">Meet Our Team</h2>
            <p className="text-lg md:text-xl text-gray-600 max-w-3xl mx-auto px-4">A dedicated group of professionals committed to your success</p>
          </motion.div>

          <div className="space-y-8">
            {team[0] && (
              <div className="flex justify-center px-4">
                <div className="w-full sm:w-80 md:w-1/3 lg:w-1/4">
                  <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="card relative overflow-hidden h-80 flex flex-col justify-end p-0 group w-full mx-auto">
                    ...
                    <div className="relative z-10 bg-black/60 w-full p-4 flex items-center justify-between">
                      <div>
                        <h3 className="text-lg font-bold text-white mb-1">{team[0].name}</h3>
                        <p className="text-gray-200 text-xs">{team[0].role}</p>
                      </div>
            ...
            {team.length > 1 && (
              <div className="flex justify-center px-4">
                <div className="w-full lg:w-3/4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
