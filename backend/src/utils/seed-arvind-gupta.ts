...
async function main() {
  console.log('🌱 Starting arvind-gupta database seed...')

  // Create arvind-gupta Website
  const arvindGupta = await prisma.website.upsert({
    where: { slug: 'arvind-gupta' },
    update: {
      name: 'Arvind Gupta',
      domain: 'https://temp9.automationcafe.in',
      ...
      themeConfig: {
        ...
        footerContent: {
          ...
          copyright: '© 2026 arvind-gupta. All rights reserved.'
        }
      },
    },
    create: {
  ...
  console.log('✅ Arvind Gupta Demo website created')

  // Helper to safely delete existing old pages to re-seed fresh
  await prisma.section.deleteMany({
    where: { page: { websiteId: arvindGupta.id } }
  })
  await prisma.page.deleteMany({
    where: { websiteId: arvindGupta.id }
  })

  // Home Page
  await prisma.page.create({
    data: {
      websiteId: arvindGupta.id,
      slug: 'home',
      title: 'Home',
      status: 'PUBLISHED',
  ...
  console.log('✅ Arvind Gupta Demo seed complete')
}
